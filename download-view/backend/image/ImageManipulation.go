...
func ByteArrayToJpg(imgfromfabric []byte, destfile string) string {
	var log = logger.BlockchainLog

	buffer := bytes.NewReader(imgfromfabric)
	img, _, err := image.Decode(buffer)
	if err != nil {
		log.Println("Decode to Image encounters Error")
		log.Println(err)
	...
	jpgpath := filepath.Join("..", "sent_jpg")
	err = os.MkdirAll(jpgpath, os.ModePerm)
	if err != nil {
		log.Println("Creating jpg directory encounters error")
		log.Println(err)
	...
	_ = jpeg.Encode(output, img, &opts)
	imgpath := filepath.Join(jpgpath, destfile)
	return imgpath
}
